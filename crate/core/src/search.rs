                        ...
                        for alias in aliases {
                            if let Some(alias_str) = alias.as_str() {
                                ...
                                if normalized_alias.contains(&normalized_query) {
                                    let score = 5000.0 + (normalized_query.len() as f64 * 10.0);
                                    let result = SearchResult {
                                        id: id.to_string(),
                                        group_id: group.group_id.clone(),
                                        aliases: aliases
                                    ...
                                    if best_match
                                        .as_ref()
                                        .is_none_or(|m: &SearchResult| m.score < score)
                                    {
                                        best_match = Some(result);
                                    }
                                }
                                ...
                                if let Some(raw_score) = self.matcher.fuzzy_match(alias_str, query)
                                {
                                    if raw_score > 0 {
